...
void test_mdfu_packet(void){
    ...
    // Encode a packet, then docode again and compare with original data
    mdfu_encode_cmd_packet(&packet, &packet_buffer);
    mdfu_packet_t decoded_packet;
    mdfu_decode_packet(&decoded_packet, MDFU_CMD, &packet_buffer);
